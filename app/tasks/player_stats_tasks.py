...
async def _update_player_stats_for_league_season(league_id: int, season: int, max_pages: int = None):
    ...
    async with session as db:
        try:
            ...
            players_updated, stats_updated = await player_service.update_player_stats_for_league_season(
...
async def _update_player_stats_for_season(season: int, max_pages: int = None):
    ...
    async with session as db:
        try:
            player_service = PlayerService()
            players_updated, stats_updated = await player_service.update_player_stats_for_season(
...
async def _update_player_stats_for_league(league_id: int, max_pages: int = None):
    ...
    async with session as db:
        try:
            player_service = PlayerService()
            players_updated, stats_updated = await player_service.update_player_stats_for_league(
